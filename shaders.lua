...
shaders.waterShader = [[
    ...
    uniform vec4 WATER_COL =  vec4(0.0,0.011764705882353,0.51764705882353,1.0);
    uniform vec4 WATER2_COL =  vec4(0.0,0.003921568627451,0.61176470588235,1.0);
    uniform vec4 FOAM_COL = vec4(0.023529411764706,0.67058823529412,0.87450980392157,1.0);
    ...
    vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 pixel_coords) {
        vec4 pixel = vec4(water((pixel_coords/400) * tile, vec3(0,1,0), time * distortion_speed), 1.0);
        return pixel;
    }
]]
...
local copilotShader = [[
    ...
    vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 pixel_coords)
    {
        ...
        pixel.r = pixel.r + sin(time + pixel_coords.x * 0.01) * 0.1;
        pixel.g = pixel.g + sin(time + pixel_coords.y * 0.01) * 0.1;
        pixel.b = pixel.b + sin(time + pixel_coords.x * 0.01) * 0.1;
        return pixel;
    }
]]
